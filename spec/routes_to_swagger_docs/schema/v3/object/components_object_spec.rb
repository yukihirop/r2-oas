...
RSpec.describe RoutesToSwaggerDocs::Schema::V3::ComponentsObject do
  let(:routes_data) do
    [
  ...
  let(:object) { RoutesToSwaggerDocs.use_object_classes[:components_object].new(routes_data) }
  ...
  describe '#to_doc' do
    context 'when use before_create && after_create' do
      before do
        class TestComponentsObject < RoutesToSwaggerDocs::Schema::V3::ComponentsObject
